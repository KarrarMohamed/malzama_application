...
import 'package:malzama_app/src/features/home/presentation/pages/my_materials/my_saved_and_uploads/my_saved_material/state_provider/saved_videos_state_provider.dart';
...
// ========================================  Functionalities ====================================
  ...
  Future<bool> uplaodNewComment({
    ...
    Map<String, String> _newCommentData = state.newCommentData..addAll({'notificationText': _getNewCommentNotificationText()});
    ContractResponse response = await CommentsClient().createNewComment(commentData: _newCommentData, content: content);
...
// ================================================================================================
  Future<void> uploadNewReply({
    @required String content,
    @required int replyPos,
    bool resend = false
  }) async {
    if(!resend){
      ...
      _comments[_repliesRelevantCommentPos].replies[replyPos].commentStatus = CommentStatus.IN_PROGRESS;
    }
    notifyMyListeners();
    ...
    if (contractResponse is Success) {
      ...
      _comments[_repliesRelevantCommentPos].replies.firstWhere((element) => element.content == content).commentStatus = CommentStatus.FAILED;
      notifyMyListeners();
    }
  }
...
// ================================================================================================
  ...
  String _getNewCommentNotificationText() {
    User userData = locator<UserInfoStateProvider>().userData;
    final String notificationTail = 'commented on your ${state.materialType}';
    final String firstCommenter = '${userData.firstName} ${userData.lastName}';
    if (_comments.isEmpty) {
      return '$firstCommenter $notificationTail';
    }
    final secondCommenter = '${_comments.first.author.firstName} ${_comments.first.author.lastName}';
    if (_comments.length == 1) {
      return '$firstCommenter and $secondCommenter  $notificationTail';
    }
    return '$firstCommenter,$secondCommenter and ${_comments.length - 1} others $notificationTail';
  }

  String _getNewReplyNotificationText(int commentIndex) {
    User userData = locator<UserInfoStateProvider>().userData;
    bool isMyMaterial = userData.id == state.author.id;
    final String firstReplier = '${userData.firstName} ${userData.lastName}';
    final String notificationTail =
        'replied to your comment on ${isMyMaterial ? 'your' : '${state.author.firstName}\'s'} ${state.materialType}';
    if (_comments[commentIndex].replies.isEmpty) {
      return '$firstReplier $notificationTail';
    }
    final String secondReplier =
        '${_comments[commentIndex].replies.first.author.firstName} ${_comments[commentIndex].replies.first.author.lastName}';
    if (_comments[commentIndex].replies.length == 1) {
      return '$firstReplier and $secondReplier $notificationTail';
    }
    return '$firstReplier,$secondReplier and ${_comments[commentIndex].replies.length - 1} others $notificationTail';
  }

  String _getNewRatingNotificationText(int commentIndex, bool ratingType) {
    User userData = locator<UserInfoStateProvider>().userData;
    String firstRater = '${userData.firstName} ${userData.lastName}';
    final String commentText =
        _comments[commentIndex].content.length > 30 ? _comments[commentIndex].content.substring(0, 31) : _comments[commentIndex].content;
    final String notificationTail = 'rated your comment \'$commentText\'';
    if (_comments[commentIndex].ratings.isEmpty) {
      return '$firstRater has $notificationTail as ${getRatingText(ratingType)}';
    }
    final String secondRater =
        '${_comments[commentIndex].ratings.first.author.firstName} ${_comments[commentIndex].ratings.first.author.lastName}';
    if (_comments[commentIndex].ratings.length == 1) {
      bool similarRating = _comments[commentIndex].ratings.first.ratingType == ratingType;
      return '$firstRater and $secondRater have $notificationTail ${similarRating ? 'as ${getRatingText(ratingType)}' : ''}';
    }
    return '$firstRater,$secondRater and ${_comments[commentIndex].ratings.length - 1} others have $notificationTail';
  }

  String getRatingText(bool value) => value ? 'helpful' : 'unhelpful';
// ================================================================================================
}
